  ...
  */
  ...
  void main() {
    vUv = uv;
    vec3 pos = vec3(position.x,position.y, u_zpos); // When using perspective projection matrix, u_zpos passed to keep mesh full screen and show animation clearly
    // vec3 pos = position;
    // For scaling down the x - coordinate system according to the aspect ratio 
    // for unstretched shaped meshes (scaling x axis for devices with longer width
